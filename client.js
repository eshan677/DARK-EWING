...
try {
    ...
    console.log("🌍 Connected to the EWING DB");
} catch {
...
let cc = Config.conf.session.replace(/DARK-EWING;;;=>/g,""); // DONT REMOVE BLACK;;; so you can connect BLACK md session id too
...
await updateStatus()
 ...
 Void.ev.on('group-participants.update', async(anu) => {
        ...
        Void.ev.on('connection.update', async(update) => {
                ...
                if (connection === 'open') {
                    console.log("✅ Ewing Login Successful!");
                    console.log("⬇️  Ewing Installing External Plugins...");
                    ...
                    for (let i = 0; i < check.length; i++) {
                     ...
                     console.log("😁  Ewing External Plugins Installed!");fs.readdirSync(__dirname + "/../commands").forEach((plugin) => {
